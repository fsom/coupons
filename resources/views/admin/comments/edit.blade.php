    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.comments.update", [$comment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('shop') ? 'is-invalid' : '' }}" name="shop_id" id="shop_id" required>
                    @foreach($shops as $id => $entry)
                        <option value="{{ $id }}" {{ (old('shop_id') ? old('shop_id') : $comment->shop->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('shop'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('stars') ? 'is-invalid' : '' }}" name="stars" id="stars" required>
                    ...
                    @foreach(App\Models\Comment::STARS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('stars', $comment->stars) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('stars'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip') ? 'is-invalid' : '' }}" type="text" name="ip" id="ip" value="{{ old('ip', $comment->ip) }}">
                @if($errors->has('ip'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('data') ? 'is-invalid' : '' }}" name="data" id="data">{{ old('data', $comment->data) }}</textarea>
                @if($errors->has('data'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('answer') ? 'is-invalid' : '' }}" name="answer" id="answer">{{ old('answer', $comment->answer) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('answer_at') ? 'is-invalid' : '' }}" type="text" name="answer_at" id="answer_at" value="{{ old('answer_at', $comment->answer_at) }}">
