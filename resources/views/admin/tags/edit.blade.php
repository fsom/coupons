...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tags.update", [$tag->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="title">{{ trans('cruds.tag.fields.title') }}</label>
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $tag->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $tag->description) }}">
            ...
            <div class="form-group">
                <label for="image">{{ trans('cruds.tag.fields.image') }}</label>
                <div class="needsclick dropzone {{ $errors->has('image') ? 'is-invalid' : '' }}" id="image-dropzone">
                </div>
                @if($errors->has('image'))
                    <span class="text-danger">{{ $errors->first('image') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.tag.fields.image_helper') }}</span>
            </div>
            <div class="form-group">
...
@if(isset($tag) && $tag->image)
      var file = {!! json_encode($tag->image) !!}
